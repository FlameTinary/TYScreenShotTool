...
### 1. AI Pro 按区域开放，开发者本地 AI 配置仍为隐藏能力

当前 `AI` 入口在区域策略允许时默认显示；中国大陆和 unknown storefront 下仍会隐藏。Debug Settings 中的开发者 AI 开关与 Debug storefront 覆盖只能表达本地调试意图，最终仍由区域策略决定是否显示和是否允许请求。

面向用户的 AI Pro 走 Apple 登录、订阅和后端校验链路。开发者本地 AI 请求配置仍然不提供完整的公开设置页入口。
