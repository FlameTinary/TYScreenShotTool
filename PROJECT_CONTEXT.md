...
- 在已建立 App 端区域策略、开发者 AI 隔离和海外 AI Pro 壳层后，继续推进 StoreKit 2 订阅接入边界
...
- 海外模式下当前只展示 AI Pro 壳层说明；点击入口不会上传截图，也不会调用 AI 后端
...
- 海外 AI Pro 壳层已可通过 Debug storefront 覆盖模拟验证，但尚未接入登录、StoreKit 或后端
