...
This directory contains the Sprint 53.5 backend MVP for the overseas AI Pro path.
...
- No real AI provider forwarding yet.

The MVP intentionally blocks AI forwarding with `501 ai_forwarding_not_implemented` after validating auth, region, subscription, quota, request shape, and image size. Feature 53.6 will add the actual AI forwarding and usage mutation path.
...
- `POST /v1/ai/analyze-screenshot`: requires Bearer token, region allowed, active subscription, available quota, valid image payload; then returns `501 ai_forwarding_not_implemented`
