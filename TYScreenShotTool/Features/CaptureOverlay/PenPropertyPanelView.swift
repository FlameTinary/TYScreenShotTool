...
/// 画笔属性面板
///
/// 在工具栏下方显示，提供大小、不透明度、颜色和模式（单色/高斯模糊/马赛克）的调整。
...
final class PenPropertyPanelView: NSVisualEffectView {
    ...
    private let lineWidthValueLabel = NSTextField(labelWithString: "16")
    ...
    private func setupControls() {
        ...
        lineWidthSlider.minValue = 4
        lineWidthSlider.maxValue = 60
        lineWidthSlider.doubleValue = 16
        ...
        modePopUp.addItems(withTitles: [
            AppText.penModeSingleColor,
            AppText.penModeGaussianBlur,
            AppText.penModeMosaic
        ])
    ...
    private func setupLayout() {
        ...
        let modeLabel = NSTextField(labelWithString: AppText.penModeSingleColor)
        ...
        let rightStack = NSStackView(views: [modePopUp])
        ...
        colorLabel.snp.makeConstraints { make in
            make.leading.equalTo(leftStack.snp.trailing).offset(12)
            make.top.equalTo(rightStack.snp.bottom).offset(12)
    ...
    @objc private func modeChanged() {
        currentProperties.mode = mode(for: modePopUp.indexOfSelectedItem)
    }
    ...
    private func mode(for index: Int) -> PenMode {
        switch index {
        case 0: return .singleColor
        case 1: return .gaussianBlur
